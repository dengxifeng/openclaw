...
Base upstream version: **2026.4.2**
RISC-V version: **2026.4.2-riscv64.1**
...
The riscv64 build uses a version suffix (`2026.4.2-riscv64.1`), but the public npm registry's `openclaw` package only provides x86-64/arm64 builds. Running `openclaw update` would pull an incompatible upstream version.
